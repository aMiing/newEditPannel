			...
			<div class="widgetGroup" style="position: absolute;left:200px;top:;z-index: 9;width:600px;text-align: center;color:#fff;">
				...
				<el-col :span="24" class="components"style="line-height:18px;">
					<el-menu class="el-menu-demo" mode="horizontal" @select="" text-color="#fff" active-text-color="#6bf" default-active="1">
					  <el-submenu :index="index" v-for="(tabItem,index) in tabItems">
					    <template slot="title"><i :class="'iconfont '+tabItem.icon" :title="tabItem.name"></i></template>
							    ...
							    <el-tab-pane label="全部" title="全部" name="all">
								  	<div class="Charts">
								  		<div class="components-item" v-for="item in widgets" v-if="item.heapId==index&&item.usable==1" @click="UseIt(item)">
								  			<img :src="item.img" :alt="item.name" :title="item.name">
								  			<p style="line-height: 16px;width:100%;margin:0;">{{item.name}}</p>
								  		</div>
								  		<div class="components-item" v-for="item in widgets" v-if="item.heapId==index&&item.usable==0">
							    ...
							    <el-tab-pane :label="child" :title="child" v-for="(child,childIndex) in tabItem.childTab">
								  	<div class="barChart">
		...
		<el-col :span="24" class="main">
			<aside id="aside-left" :class="collapsed1?'menu-collapsed':'menu-expanded'" style="">
		        <div class="side-head" style="width:100%;text-align: center;">
		        	<h3 style="margin:10px 0;background-color: #">
		        		<span>控件列表</span>
		        	</h3>
...
<script>
	...
	export default {
		...
		created:function(){
			...
			//加载所有控件;加载页面绑定控件//ajax获取另一个服务上的数据
	    	var url = (window.location.href).replace('8080/#','8888');
	    	console.log(url);  
	    	this.$http.get(url).then(function(res){
	    		let res_arr = (res.bodyText).split(';');//两部分数据混在一起，进行拆分
	    		console.log(res_arr);
	    		let Widgets = JSON.parse(res_arr[0]);//解析widgets
	    		this.widgets = Widgets;
	    		console.log(this.widgets);
	    		let BindWidget = JSON.parse(res_arr[1]);
	    		let screenSetting = JSON.parse(res_arr[2]);
	    		let tabItemsJson = JSON.parse(res_arr[3]);
	    		for (var i = 0; i < tabItemsJson.length; i++) {
	    			console.log(tabItemsJson[i].childTab);
	    			if(tabItemsJson[i].childTab != ''){
	    				tabItemsJson[i].childTab = (tabItemsJson[i].childTab).split(',');
	    			}else{
	    				// tabItemsJson[i].childTab = '';
	    			}
	    		}
	    		let _self = this;
	    		setTimeout(function(){
					_self.tabItems = tabItemsJson;
					console.log(_self.tabItems);
	    		},100);

	    		
	    		this.screenSet= screenSetting[0];
	    		// console.log(this.screenSet);
	    		this.ScreenBgColor = this.screenSet.bgcolor;//将拿到的数据放在暂存的数据字段里
	    		this.screenBgImg = this.screenSet.bgimg;//将拿到的数据放在暂存的数据字段里
		...
		methods: {
			saveChange(){
				/*	let url_saveScreen = 'http://localhost:8888/saveScreenSetting';
					...
					if(this.pageComponents != ''){
						...
						self.$http.post(url,{"str":newJsonStr},{emulateJSON:true}).then((req,res) => {
		    				console.log(res.bodyText);
				    	}).catch((err) =>{
				    		console.log('确实修改数据库成功了，但是这了还是报了个错');
				    		return;
			...
			changeSize(size){
				...
				for (let i = 0; i < self.pageComponents.length; i++) {
					if(self.pageComponents[i].uid == size.uid){
						...
						self.pageComponents[i].widgetHeight = Math.round(size.height*10)/10;
						return;
					}
				}
			...
			changePosition(position){
				...
				for (let i = 0; i < self.pageComponents.length; i++) {
					if(self.pageComponents[i].uid == position.uid){
						self.pageComponents[i].widgetLeft = Math.round(position.left*10)/10;
			...
			deleate(theId){
				...
				this.$http.post(url,{"id":theId},{emulateJSON:true}).then((req,res) => {
    				console.log(res.bodyText);
		    	}).catch((err) =>{
		    		console.log('未能成功发送新增widget数据，请联系系统维护人员或稍后重试！');
		    		return;
		    	});
		    	//遍历数组，删除选定元素
		    	let self = this;
			...
			previewIt(){
				...
				var content = document.getElementsByClassName('screenPannel')[0].innerHTML;
				console.log(content);
	    		let pageId = ((window.location.href).split('='))[1];
				//1、上传数据到服务器思路
					let url = 'http://localhost:8888/previewScreen';
					this.$http.post(url,{id:pageId,content:content},{emulateJSON:true}).then((req,res) => {
	    				console.log(res.bodyText);
			    	}).catch((err) =>{
			    		console.log('未能成功发送预览界面数据，请联系系统维护人员或稍后重试！');
			    		return;
			    	});
			    //跳转预览界面
			...
			UseIt(item,index){
	    		i = (this.pageComponents.length != 0) ? parseInt((this.pageComponents[this.pageComponents.length-1]).uid.slice(4,))+1 : this.pageComponents.length;
	    		console.log("i:"+i);
				//i++;//重要参数
				...
				this.$http.post(url,{"uid":'uid_'+i,"name":item.name,"bindId":bindId,"styleJson":item.styleJson,"dataJson":item.dataJson,"layer":i},{emulateJSON:true}).then((req,res) => {
    				console.log(res.bodyText);
		    	}).catch((err) =>{
		    		console.log('未能成功发送新增widget数据，请联系系统维护人员或稍后重试！');
		    		return;
		    	...
		    	// console.log(url);  
		    	this.$http.get(reload_url).then(function(res){
		    		let res_arr = (res.bodyText).split(';');//两部分数据混在一起，进行拆分
		    		console.log(res_arr);
		    		let BindWidget = JSON.parse(res_arr[1]);
...
<style scoped lang="scss">
	...
	*{
	...
	.components-item[data-v-498a33db],.target-item[data-v-498a33db]{
		color:#999;
	} 
	.components-item[data-v-498a33db]:hover,.target-item[data-v-498a33db]:hover{
		color:#e91;
	}
	.el-menu-item, .el-submenu__title{
		color:#eceff3;
